...
class MyProfitController extends GetxController {
  int _current = 1;
  int _pageSize = 10;
  bool isLoading = false;
  bool isLoadAll = false;
  String _walletId;
  List<ProfitVo> dataList = [];
  ...
  void queryList({String walletId}) async {
    try {
      ...
      isLoading = true;
      final res = await queryMyProfitPage(_walletId, data: {
        'current': _current,
        'pageSize': _pageSize,
      });
      final data = res.data['data'];
      List items = data['data'] ?? [];
      List<ProfitVo> _dataList =
          items.map((item) => ProfitVo.fromJson(item)).toList();
