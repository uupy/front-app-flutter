
import 'package:get/get.dart';
import 'package:wine/api/account.dart';
import 'package:wine/common/app.dart';
import 'package:wine/src/account/list/model.dart';
class AccountListController extends GetxController{
  List<Account> dataList = [];
  int _current = 1;
  int _pageSize = 20;
  bool isLoadAll = false;
  bool isLoading = false;

  queryList() async{
    isLoading = true;
    var res = await getAccountPage(
      {
        'current': _current,
        'pageSize': _pageSize
      }
    );
    List list = res.data['data']['data'] ?? [];
    List<Account> items = list.map((item) => Account.fromJson(item)).toList();
    if (_current == 1) {
      dataList = items;
    } else {
      dataList = [...dataList, ...items];
    }
    isLoading = false;
    print('----------- queryList -------------: $dataList');
    update();
  }

  void remove(id) async{
    await removeAccount(id);
    app.showToast('已删除');
    Get.back();
    reload();
  }

  Future reload() async{
    _current = 1;
    queryList();
  }

  void loadMore() async{
    if (!isLoadAll && !isLoading) {
      _current++;
      queryList();
    }
  }

  void resetAccountPassword(userId) async{
    await resetPassword(userId);
    app.showToast('密码已重置');
    Get.back();
    reload();
  }
}